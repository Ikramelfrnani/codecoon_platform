...
const fulltext = `Before we continue, let's complete some questions to allow us know more about you
so we can make this journey truly yours.`;
...
const Continue = () => {
  ...
  const handleContinue = async () =>{
    const emptyFieldRes = await api.get(`/api/utilisateurs/${utilisateur_id}/empty-fields`);
    const { empty_field } = emptyFieldRes.data;
    const questionIndex = questions.findIndex(q => q.field === empty_field);
    const question = questions[questionIndex];

    navigate(`/onboard/${question.slug}`);
  }
