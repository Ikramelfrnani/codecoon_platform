...
const NavBar = () => {
  ...
  const [pointsEnergie, setPointsEnergie] = useState(0);
  ...
  const fetchBadgeCount = async () => {
    try {
      ...
      const response = await api.get(`/api/utilisateur/${utilisateur_id}/badges/count`);
      setBadgeCount(response.data.badges_count);
    } catch (error) {
...
useEffect(() => {
  const fetchPointsEnergie = async () => {
    try {
      const utilisateur_id = localStorage.getItem('utilisateur_id');
      if (utilisateur_id) {
        const response = await api.get(`/api/utilisateurs/${utilisateur_id}`);
        setPointsEnergie(response.data.points_energie);
      }
    } catch (error) {
      console.error('Failed to fetch points_energie:', error);
    }
  };
    ...
    <NavbarWrapper>
      <NavbarContainer>
        ...
        <RightContainer>
          <ButtonsContainer>
            <Button as="span">
              <Count>{pointsEnergie}</Count>
              <BadgeIcon src="/images/coiny.svg" alt="Coin" />
              
            </Button>
            <Button to="/badges">
              <Count>{badgeCount}</Count>
              <BadgeIcon
                src={badgeCount > 0 ? "/images/codecoonbadge.svg" : "/images/defaultbadge.svg"}
                alt="Badge"
              />
            </Button>
